module.exports = (site, baseUrl, pass) => {
  Cypress.on('uncaught:exception', (err, runnable) => {
    ...
    //     cy.log('3️⃣ Check property links exist and have valid hrefs');
    //     findElementWithSelectors(propertyLinkSelectors).each(($el, index) => {
    //         const href = $el.prop('href');
    //         cy.log(`📝 Checking property link #${index + 1}: ${href}`);
            
    //         // Just verify the link exists and has an href - don't visit it
    //         cy.wrap($el)
    //         .should('have.attr', 'href')
    //         .and('not.be.empty');
            
    //         // Optional: Make a simple HTTP request to check if URL is reachable
    //         cy.request({
    //         url: href,
    //         failOnStatusCode: false // Don't fail the test if URL returns error
    //         }).then((response) => {
    //         if (response.status >= 200 && response.status < 400) {
    //             cy.log(`✅ Property link is reachable: ${href} (Status: ${response.status})`);
    //         } else {
    //             cy.log(`⚠️ Property link returned status ${response.status}: ${href}`);
    //         }
    //         });
    //     });
    ...
    //         cy.log('✅ "View All" button exists and has valid href');
    //         } else {
    //         cy.log('ℹ️ No "View All" button found - this is optional');
    //         }
    //     });

    //     cy.log('✅ All featured property links have been validated without cross-origin navigation.');
    // });
    ...
    it('🏛️ Featured Communities Are Present', () => {
      ...
      const communityLinkSelectors = [
        ...
        'a.communities__slider-item',
        '.fc-slide a',
        '.fc-item a',
        '.featured-communities a',
        '.featuredCommunities__col a',
        '.community-card a',
        '.community-item a',

...
it('🏢 Featured Properties Are Present', () => {
      ...
      // Helper function to check property details on any origin
      const checkPropertyDetails = (href, propertyDetailSelectors, isCurrentOrigin = true) => {
        if (isCurrentOrigin) {
          // Same origin - use regular commands
          cy.get('body').then(($body) => {
            ...
            if (detailsFound) {
              cy.scrollTo('top', { duration: 1000 }).wait(1000);
              cy.scrollTo('bottom', { duration: 1000 }).wait(1000);
              cy.scrollTo('center', { duration: 1000 }).wait(1000);
          ...
          // Cross-origin - handle with cy.origin
          cy.url().then((currentUrl) => {
            const currentOrigin = new URL(currentUrl).origin;
            
            cy.origin(currentOrigin, { args: { propertyDetailSelectors, href } }, ({ propertyDetailSelectors, href }) => {
              cy.get('body').then(($body) => {
                let detailsFound = false;
                ...
                if (detailsFound) {
                  ...
                  cy.log(`❌ Property failed - missing property details section: ${href}`);
                  // Note: Cannot modify failedProperties from within cy.origin
                  // This limitation needs to be handled at a higher level
                }
              });
            });
          });
      ...
      // Store the current URL to determine origin differences
      cy.url().then((homePageUrl) => {
        ...
        findElementWithSelectors(propertyLinkSelectors).each(($el, index) => {
          ...
          // Check current URL and determine if cross-origin
          cy.url().then((currentUrl) => {
            const currentOrigin = new URL(currentUrl).origin;
            const isSameOrigin = currentOrigin === homeOrigin;
            
            cy.log(`🔍 Current origin: ${currentOrigin}, Home origin: ${homeOrigin}, Same origin: ${isSameOrigin}`);
            
            if (isSameOrigin) {
              // Same origin - use regular Cypress commands
              checkPropertyDetails(href, propertyDetailSelectors, true);
            } else {
              // Different origin - use cy.origin wrapper
              cy.origin(currentOrigin, { args: { propertyDetailSelectors, href } }, ({ propertyDetailSelectors, href }) => {
                cy.get('body').then(($body) => {
                  let detailsFound = false;
                  ...
                  if (detailsFound) {
                    ...
                    cy.log(`❌ Property failed - missing property details section: ${href}`);
                    // Note: Cannot modify failedProperties array from within cy.origin
                    // Consider using cy.task() to log failures to a file or external system
                  }
                });
              });
            }
        ...
        // Navigate back to homepage after checking all properties
      ...
      findElementWithSelectors(featuredSectionSelectors).then(($section) => {
        ...
        if (viewAllButtonFound) {
          ...
          // Handle cross-origin for the "View All" page as well
          cy.url().then((viewAllUrl) => {
            cy.url().then((homePageUrl) => {
              const viewAllOrigin = new URL(viewAllUrl).origin;
              const homeOrigin = new URL(homePageUrl).origin;
              
              if (viewAllOrigin !== homeOrigin) {
                // Cross-origin "View All" page
                cy.origin(viewAllOrigin, () => {

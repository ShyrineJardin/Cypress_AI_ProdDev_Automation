...
it('🏢 Featured Properties Are Present', () => {
      ...
      // Helper function to check if element exists (without throwing error)
      const checkElementExists = (selectors) => {
        return cy.window().then(() => {
          for (const selector of selectors) {
            const element = Cypress.$(selector);
            if (element.length > 0) {
          ...
          return { found: false, selectors: selectors };
        });
      };
      ...
      findElementWithSelectors(featuredSectionSelectors).within(() => {
        checkElementExists(viewAllButtonSelectors).then((result) => {
          if (result.found) {
            cy.get(result.selector)
              .should('exist')

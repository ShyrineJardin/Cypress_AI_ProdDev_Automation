...
it('🏢 Featured Properties Are Present', () => {
      ...
      cy.url().then((homePageUrl) => {
        ...
        findElementWithSelectors(propertyLinkSelectors).each(($el, index) => {
          ...
          // Check if we're on a different origin (subdomain)
          cy.url().then((currentUrl) => {
            const currentOrigin = new URL(currentUrl).origin;
            
            if (currentOrigin !== homeOrigin) {
              // Handle cross-origin scenario
              cy.log(`🌐 Cross-origin detected: ${currentOrigin} vs ${homeOrigin}`);
              ...
              cy.origin(currentOrigin, { args: { propertyDetailSelectors, href } }, ({ propertyDetailSelectors, href }) => {
                cy.get('body').then(($body) => {
                  ...
                  if (detailsFound) {
                    ...
                    // For cross-origin, we'll handle failure tracking differently
                    // Create a custom command or use task to communicate back
                  }
                });
              });
              
              // Return to home page after cross-origin check
          ...
          // After all properties are checked, scroll back to featured section
          findElementWithSelectors(featuredSectionSelectors)
            .should('exist')
            .scrollIntoView();
          cy.wait(500);
